...
(define (dec2bin n)
  (if (< n 2)
      (list n)
      (append (dec2bin (quotient n 2)) (list (modulo n 2)))))
...
(define (hex2dec num)
  ...
  (define (hex-digit-to-int n)
    (if (number? n)
        n
        (cond
          [(or (equal? n 'A) (equal? n 'a))
           10]
          [(or (equal? n 'B) (equal? n 'b))
           11]
          [(or (equal? n 'C) (equal? n 'c))
           12]
          [(or (equal? n 'D) (equal? n 'd))
           13]
          [(or (equal? n 'E) (equal? n 'e))
           14]
          [(or (equal? n 'F) (equal? n 'f))
           15])))
  (cond
    [(equal? num (list ))
     0]
    [else
     ...
     (define (subtract-one-and-power constant) 
      (set! exponent (- exponent 1))
      (* (hex-digit-to-int constant) (expt 16 exponent)))
     (apply + (map subtract-one-and-power num))]))
...
(define (dec2hex n)
  ...
  (define (int-to-hex-digit n)
    (if (< n 10)
        n
        (list-ref char-digits (- n 10))))
  (if (< n 16)
      (list (int-to-hex-digit n))
      (append (dec2hex (quotient n 16)) (list (int-to-hex-digit (modulo n 16))))))
