...
(define (reg-exp? value)
  (cond
    [(ok-string? value)
     #t]
    [(concat? value)
     (and (reg-exp? (concat-arg1 value)) (reg-exp? (concat-arg2 value)))]
    [(either? value)
     (and (reg-exp? (either-arg1 value)) (reg-exp? (either-arg2 value)))]
    [(repeat? value)
...
(define (generate-string-from-reg-exp exp)
  (cond
    [(ok-string? exp)
     exp]
    [(concat? exp)
     (append (generate-string-from-reg-exp (concat-arg1 exp)) (generate-string-from-reg-exp (concat-arg2 exp)))]
    [(either? exp)
     (pick (list (generate-string-from-reg-exp (either-arg1 exp)) (generate-string-from-reg-exp (either-arg2 exp))))]
    [(repeat? exp)
     (if (flip 0.5)
...
(define (dfa-accepts?-helper mach str state)
  (cond
    [(and (empty? str)
          (ormap (lambda (accepting-state)
                   (equal? state accepting-state))
                 (dfa-accepting-states mach)))
     #t]
    [(empty? str)
     #f]
    [(ormap (lambda (entry1)
              (equal? (list state (first str)) (entry-key entry1)))
            (dfa-transitions mach))
     (define transition1 (findf (lambda (entry1)
                                  (equal? (list state (first str)) (entry-key entry1)))
                                (dfa-transitions mach)))
     (dfa-accepts?-helper mach (rest str) (entry-value transition1))]
    [else
     #f]))
...
(define (list-leaf-labels tree)
  (cond
    [(leaf? tree)
     (list (leaf-label tree))]
    [(node? tree)
     (foldr append '() (map list-leaf-labels (node-children tree)))]))
